...
class MagmaWidget extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.$triggerId = this.getAttribute("magma-trigger-id");
  ...
  createButton() {
    ...
    if (this.hasAttribute("magma-hidden")) {
    ...
    if (this.getAttribute("custom-css-name")) {
      this.importStyle(customCss[this.getAttribute("custom-css-name")]);
    }
  ...
  getOrigin() {
    ...
    return !origin ? null : new URL(origin)?.hostname;
  }

  getReferrer() {
    const referrer = document?.referrer;

    return !referrer ? null : new URL(referrer)?.hostname;
  }
...
customElements.define("magma-widget", MagmaWidget);
